...
All five fields are required. Values are validated against `cards_schema.json` (schema-driven validation — adding a new theme means editing one JSON file, not changing Python). Unknown extra fields are ignored (not persisted).
